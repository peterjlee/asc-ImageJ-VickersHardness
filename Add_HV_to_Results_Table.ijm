...
macro "Add_HV_to_Results_Table" {
	macroL = "Add_HV_to_Results_Table_v230531-f4.ijm";
	...
	if (pixelWidth==1 || indexOfArray(availableUnits, unit, -1)==-1){
		Dialog.create("Manual scale entry");
			Dialog.addMessage("No images open, or scale appears to be non-metric, please enter unit values");
	...
	Dialog.create("Provide Load and Set Scale to Match Results Table \(" + macroL + "\)");
		...
		Dialog.addMessage("Optional overlay lines \(set width to 0 for no lines\):");
		/* ROI Feret */
		if(overlayN > 0){
		...
		Dialog.setInsets(5,0,-3);
		Dialog.addMessage("Optional Primary Feret line", 13, "#006db0");
		...
		/* ROI Feret 2 */
		Dialog.setInsets(5,0,-3);
		Dialog.addMessage("Optional Feret2 \(complimentary\) line", 13, "#006db0");
		...
		if (pixelAR!=1) Dialog.addMessage("Aspect pixels, average pixel size of " + lcf + " " + unit + " used");
		if (unit!="mm") Dialog.addMessage("Image scale in " + unit + ", so the mm" + sup2 + " scale factor used is " + sF);
	...
	if (unit!="mm") for (i=0; i<nTable; i++) avgFerets_mm[i] = avgFerets[i]*sF;
